...
namespace database {
    ...
    struct TripItem {
        TripItem() = default;
        TripItem(Stop* f, Stop* t, Bus_rout* r, TripTime trip_info) : from(f), to(t), route(r), Trip_item(trip_info) {}
        Stop* from = nullptr;
        Stop* to = nullptr;
        Bus_rout* route = nullptr;
        TripTime Trip_item = {};
    };
    ...
    public:
        transport_router(TransportCatalogue& catalogue, double bus_wait_time, double bus_velocity) : catalogue_(catalogue) {
            ...
            for (Bus_rout* R : this->catalogue_.get_base_routes_uniuqe()) {
                distances_range(R);
        ...
        void distances_range(const Bus_rout* rout);

        double distanceBetween(const size_t from, const size_t to);

        void addTripItem(Stop* from, Stop* to, Bus_rout* route, TripTime&& trip_info);
